Module HelperFunctions
    ...
    Function writeMessage(ByVal message As String, ByVal streamName As String, ByVal username As String) 'when you send a message it needs to write to the database
        ...
        Try
            writeSQL("insert into tbl_messages (StreamID, FromID, Timestamp, Message) values ('" & StreamID & "', '" & fromID & "', getdate(), '" & MakeSQLSafe(Filter(message)) & "')")
        Catch ex As Exception
            'console.writeline(ex.ToString)
            'console.writeline(ex.ToString)
    ...
    'insert the whole string in _swear, that was generated by the previous project
    Private _swear As String = "shit, ass, Mr. Brasher, etc" '<-insert string text here
    Private _swearArray As String() = _swear.Split(","c)

    Public Function Filter(ByVal textBoxInput As String) As String
